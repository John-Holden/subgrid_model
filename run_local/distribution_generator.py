...
settings = {"out_path": os.getcwd() + '/animationsData/', "anim": False, "BCD3": False, "verbose": False,
            "save_figs": False, "plt_tseries": True, "dyn_plots": [False, 1, False],
            "R0_mode": False}
#
...
params["domain_sz"] = [100, 100]  # Size of domain/Lattice boundaries (x\alpha for m^2)
...
if settings["R0_mode"]:  # Test R0
    ...
    settings["BCD3"] = True
    settings["dyn_plots"] = [True, 1, False]
    settings["anim"] = True
...
for i in range(ensemble_size):
    ...
    if settings["R0_mode"]:
        ensemble_results[i] = results[0]  # Save mortality (all others fields NA)
    elif not settings["R0_mode"]:
        ensemble_results[i] = results[4]  # Save result X
        print('perc: ', results[4])
...
pr_extinction = len(np.where(ensemble_results == 0)[0]) / ensemble_size+1
