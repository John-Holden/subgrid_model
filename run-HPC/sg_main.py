...
"""
settings["HPC"] = mode
settings["BCD3"] = True  # IF false --> mortality sims i.e. run-HPC until all trees are dead
...
rho_Arr = np.array([0.025, 0.050])
beta_Arr = np.linspace([0.0001, 0.0010, 50])
eff_sigma_Arr = np.array([40, 50, 60]) / alpha  # Dispersal distance in comp units (not physical
